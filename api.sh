...
# 1. Directories
mkdir -p \
  "$BASE/src/DataPipelineApi (python port)/Controllers" \
  "$BASE/src/DataPipelineApi (python port)/Models" \
  "$BASE/src/DataPipelineApi (python port)/Options" \
  "$BASE/src/DataPipelineApi (python port)/Services"
...
# 3. csproj
cat > "$BASE/src/DataPipelineApi (python port)/DataPipelineApi (python port).csproj" << 'EOF'
...
# 4. Program.cs
cat > "$BASE/src/DataPipelineApi (python port)/Program.cs" << 'EOF'
using DataPipelineApi (python port).Options;
using DataPipelineApi (python port).Services;
...
# 5. Options
declare -a opts=(
  "DatabaseOptions|namespace DataPipelineApi (python port).Options; public class DatabaseOptions { public string MySql { get; set; } = \"\"; public string Postgres { get; set; } = \"\"; }"
  "MinioOptions|namespace DataPipelineApi (python port).Options; public class MinioOptions { public string Endpoint { get; set; } = \"\"; public string AccessKey { get; set; } = \"\"; public string SecretKey { get; set; } = \"\"; public string BucketRaw { get; set; } = \"\"; public string BucketProcessed { get; set; } = \"\"; }"
  "KafkaOptions|namespace DataPipelineApi (python port).Options; public class KafkaOptions { public string BootstrapServers { get; set; } = \"\"; public string Topic { get; set; } = \"\"; }"
  "AirflowOptions|namespace DataPipelineApi (python port).Options; public class AirflowOptions { public string BaseUrl { get; set; } = \"\"; public string Username { get; set; } = \"\"; public string Password { get; set; } = \"\"; }"
  "GEOptions|namespace DataPipelineApi (python port).Options; public class GEOptions { public string CliPath { get; set; } = \"\"; }"
  "AtlasOptions|namespace DataPipelineApi (python port).Options; public class AtlasOptions { public string Endpoint { get; set; } = \"\"; public string Username { get; set; } = \"\"; public string Password { get; set; } = \"\"; }"
  "MLflowOptions|namespace DataPipelineApi (python port).Options; public class MLflowOptions { public string TrackingUri { get; set; } = \"\"; }"
  "GitHubOptions|namespace DataPipelineApi (python port).Options; public class GitHubOptions { public string ActionsApi { get; set; } = \"\"; public string Token { get; set; } = \"\"; }"
)

for o in "${opts[@]}"; do
  IFS="|" read -r name body <<< "$o"
  cat > "$BASE/src/DataPipelineApi (python port)/Options/${name}.cs" << EOF
${body}
EOF
done

# 6. Models
cat > "$BASE/src/DataPipelineApi (python port)/Models/BatchRequest.cs" << 'EOF'
namespace DataPipelineApi (python port).Models;
...
cat > "$BASE/src/DataPipelineApi (python port)/Models/StreamingRequest.cs" << 'EOF'
namespace DataPipelineApi (python port).Models;
...
# 7. Services
# 7.1 DbService
cat > "$BASE/src/DataPipelineApi (python port)/Services/IDbService.cs" << 'EOF'
using System.Collections.Generic;
namespace DataPipelineApi (python port).Services;
...
cat > "$BASE/src/DataPipelineApi (python port)/Services/DbService.cs" << 'EOF'
using Dapper;
...
# 7.2 MinioService
cat > "$BASE/src/DataPipelineApi (python port)/Services/IStorageService.cs" << 'EOF'
using System.IO;
namespace DataPipelineApi (python port).Services;
...
cat > "$BASE/src/DataPipelineApi (python port)/Services/MinioService.cs" << 'EOF'
using System.IO;
using Amazon;
using Amazon.S3;
...
# 7.3 KafkaService
cat > "$BASE/src/DataPipelineApi (python port)/Services/IKafkaService.cs" << 'EOF'
namespace DataPipelineApi (python port).Services;
...
cat > "$BASE/src/DataPipelineApi (python port)/Services/KafkaService.cs" << 'EOF'
...
cat > "$BASE/src/DataPipelineApi (python port)/Services/IGEValidationService.cs" << 'EOF'
namespace DataPipelineApi (python port).Services;
...
cat > "$BASE/src/DataPipelineApi (python port)/Services/GEValidationService.cs" << 'EOF'
...
# 7.5 BatchService
cat > "$BASE/src/DataPipelineApi (python port)/Services/IBatchService.cs" << 'EOF'
namespace DataPipelineApi (python port).Services;
...
cat > "$BASE/src/DataPipelineApi (python port)/Services/BatchService.cs" << 'EOF'
...
cat > "$BASE/src/DataPipelineApi (python port)/Services/IStreamingService.cs" << 'EOF'
namespace DataPipelineApi (python port).Services;
...
cat > "$BASE/src/DataPipelineApi (python port)/Services/StreamingService.cs" << 'EOF'
using System.Net.Http.Headers;
using System.Text;
using Microsoft.Extensions.Options;
using DataPipelineApi (python port).Options;
...
# 7.7 AtlasService
cat > "$BASE/src/DataPipelineApi (python port)/Services/IAtlasService.cs" << 'EOF'
namespace DataPipelineApi (python port).Services;
...
cat > "$BASE/src/DataPipelineApi (python port)/Services/AtlasService.cs" << 'EOF'
using System.Net.Http.Headers;
using Microsoft.Extensions.Options;
using DataPipelineApi (python port).Options;
...
# 7.8 MLflowService
cat > "$BASE/src/DataPipelineApi (python port)/Services/IMLflowService.cs" << 'EOF'
namespace DataPipelineApi (python port).Services;
...
cat > "$BASE/src/DataPipelineApi (python port)/Services/MLflowService.cs" << 'EOF'
using Microsoft.Extensions.Options;
using DataPipelineApi (python port).Options;
...
# 7.9 CIService
cat > "$BASE/src/DataPipelineApi (python port)/Services/ICIService.cs" << 'EOF'
namespace DataPipelineApi (python port).Services;
...
cat > "$BASE/src/DataPipelineApi (python port)/Services/CIService.cs" << 'EOF'
using System.Net.Http.Headers;
using Microsoft.Extensions.Options;
using DataPipelineApi (python port).Options;
...
cat > "$BASE/src/DataPipelineApi (python port)/Services/IMonitoringService.cs" << 'EOF'
namespace DataPipelineApi (python port).Services;
...
cat > "$BASE/src/DataPipelineApi (python port)/Services/MonitoringService.cs" << 'EOF'
namespace DataPipelineApi (python port).Services;
...
# 8. Controllers
# 8.1 BatchController
cat > "$BASE/src/DataPipelineApi (python port)/Controllers/BatchController.cs" << 'EOF'
using DataPipelineApi (python port).Models;
using DataPipelineApi (python port).Services;
using Microsoft.AspNetCore.Mvc;
using System.IO;
using System.Text.Json;

namespace DataPipelineApi (python port).Controllers;
[ApiController]
[Route("api/batch")]
...
cat > "$BASE/src/DataPipelineApi (python port)/Controllers/StreamingController.cs" << 'EOF'
using DataPipelineApi (python port).Models;
using DataPipelineApi (python port).Services;
using Microsoft.AspNetCore.Mvc;
using System.Text.Json;
...
cat > "$BASE/src/DataPipelineApi (python port)/Controllers/GovernanceController.cs" << 'EOF'
using Microsoft.AspNetCore.Mvc;
using DataPipelineApi (python port).Services;
...
cat > "$BASE/src/DataPipelineApi (python port)/Controllers/MonitoringController.cs" << 'EOF'
using Microsoft.AspNetCore.Mvc;
using DataPipelineApi (python port).Services;
...
# 8.5 MLController
cat > "$BASE/src/DataPipelineApi (python port)/Controllers/MLController.cs" << 'EOF'
using Microsoft.AspNetCore.Mvc;
using DataPipelineApi (python port).Services;
...
# 8.6 CIController
cat > "$BASE/src/DataPipelineApi (python port)/Controllers/CIController.cs" << 'EOF'
using Microsoft.AspNetCore.Mvc;
using DataPipelineApi (python port).Services;
...
WORKDIR /src
COPY ["src/DataPipelineApi (python port)/DataPipelineApi (python port).csproj","DataPipelineApi (python port)/"]
RUN dotnet restore "DataPipelineApi (python port)/DataPipelineApi (python port).csproj"
COPY src/DataPipelineApi (python port)/. ./DataPipelineApi (python port)
WORKDIR /src/DataPipelineApi (python port)
...
EXPOSE 80
ENTRYPOINT ["dotnet","DataPipelineApi (python port).dll"]
EOF
